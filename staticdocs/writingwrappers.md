...
Writing your own wrapper is also possible, and not too complicated. It is a simple S3 Object. If ou do not know about S3 Object, you might similarly think about providing several callback functions to the **SVMBridge**. The most complex part will be provide functions to read and write the model. As a starter, you might want to look into the wrappers, **SVMBridge** provides.

The following parts have to be written where needed
...
This will only put together the command line arguments needed for training.
Therefore it takes many options ("...") and returns a simple string.
As training is done via command line, the data must be available on disk,
not memory. The ```createTrainingArguments``` method also needs to
specify where the model will be found after training.
