...
#' Generate data set with all information stored in the first two components
#'
...
#' TODO: cite paper properly
#'
#' @param		filePath		where to save the example file (as sparse data)
generateConcentratedInformationExample <- function (filePath = NULL, n = 50, eps = 0.01, seed = NULL, normalize = TRUE, verbose = FALSE)
{
	# set seed
    ...
    if (is.null (filePath) == FALSE) {
		...
		if (verbose == TRUE)
			cat("Writing concentrated information example with ", n, " examples to ",  filePath, "\n")
		writeLines (df, con)
		close(con)
    }
...
# generate +/-1 examples on (0,+/-n)
generateEasyExample2D <- function (filePath = "./easyexample2D.data", n = 1)
{
  # open connection  
  filePath = file.path(dirname(filePath), paste (basename(filePath), "_", toString(n), sep = ""))
  con  <- file(filePath, open = "w")
  messagef ("\t\tWriting easy example with %d examples to %s", n, filePath)
	...
	# a lot of points just behind these
	for (i in seq (0,ceiling(n/4))) {
		writeLines( paste( "1 1:", toString((i+2)), "2:1", sep = ""), con)
		writeLines( paste( "1 1:", toString((i+2)), "2:-1", sep = ""), con)
		writeLines( paste( "-1 1:", toString(-(i+2)), "2:1", sep = ""), con)
		writeLines( paste( "-1 1:", toString(-(i+2)), "2:-1", sep = ""), con)
	}
