...
# compute primary optimization minimum for LLSVM
computeOptimizationValuesLLSVM <- function (modelData = "", 
    trainingDataPath = "", 
    predictionOutput = "",
    verbose = FALSE, 
    data = NULL)
{
    ...
    # get the modelmatrix and compute the 'true' alphas from it
    M = matrix(newmodel$a, byrow = TRUE)
    M = (matrix (M, nrow = sqrt(dim(M)[1])))
    w = matrix(newmodel$w, byrow = TRUE)
    newmodel$a = M %*% w

    ULTRAVERBOSE = TRUE
    
    if (ULTRAVERBOSE == TRUE)
    {
        print ("M --> ")
        print(M)
        print ("w --> ")
...
computeOptimizationValuesLibSVMx <- function (model, trainingDataPath, 
        data = NULL, 
        ...
        verbose = FALSE)
{
    ...
    C = model$C
    bias = model$bias
    if (verbose)  messagef("  bias:  %f", model$bias)
